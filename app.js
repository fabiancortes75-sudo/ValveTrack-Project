...
function render() {
    ...
    if (state.currentView === 'dashboard') {
        ...
        renderAlerts(container, typeFilteredValves);
    } else if (state.currentView === 'upcoming') {
        renderUpcoming(container, typeFilteredValves);
    }
        ...
        <div class="data-card" style="border-left: 5px solid var(--danger);">
            <div class="table-header">
                <h3 style="color: var(--danger); display: flex; align-items: center; gap: 0.5rem;">
                    <i data-lucide="alert-octagon"></i> Alertas Críticas (Vencidas)
                </h3>
                <div class="badge" style="background: var(--danger)">${expired.length}</div>
            </div>
            <div class="table-container">
                <table>
                    <thead>
                    ...
                    <tbody>
                        ${expired.map(v => `
                            <tr>
                                <td style="font-family: monospace;">${v.erpId}</td>
                                <td>${v.equipment}</td>
                                <td style="color: var(--danger); font-weight: 700;">${v.scheduledDate}</td>
                                <td>
                                    <button class="action-link edit-valve" data-id="${v.id}" style="color: var(--danger)">
                                        <i data-lucide="refresh-cw"></i> Actualizar
                                    </button>
                                </td>
                            </tr>
                        `).join('') || '<tr><td colspan="4" style="text-align: center; padding: 3rem; color: var(--text-secondary)">No hay alertas críticas en este momento.</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
function renderUpcoming(container, valves) {
    const now = new Date();
    const alertWindowDays = 30;
    const alertWindowDate = new Date();
    alertWindowDate.setDate(now.getDate() + alertWindowDays);

    const upcomingRange = valves.filter(v => {
        if (v.state === 'VENCIDO') return false;
        const scheduledDate = parseSheetDate(v.scheduledDate);
        return scheduledDate && scheduledDate > now && scheduledDate <= alertWindowDate;
    });
    ...
    container.innerHTML = `
        <div class="data-card" style="border-left: 5px solid var(--warning);">
            <div class="table-header">
                <h3 style="color: var(--warning); display: flex; align-items: center; gap: 0.5rem;">
                    <i data-lucide="clock"></i> Próximos Mantenimientos (30 días)
                </h3>
                <div class="badge" style="background: var(--warning); color: #000;">${upcomingRange.length}</div>
            </div>
            <div class="table-container">
                <table>
                    <thead>
        ...
        return `
                                <tr>
                                    <td style="font-family: monospace;">${v.erpId}</td>
                                    <td>${v.equipment}</td>
                                    <td style="color: var(--warning); font-weight: 700;">${v.scheduledDate}</td>
                                    <td>${diffDays} días</td>
                                    <td>
                                        <button class="action-link edit-valve" data-id="${v.id}" style="color: var(--warning)">
                                            <i data-lucide="calendar"></i> Programar
                                        </button>
                                    </td>
                                </tr>
                            `;
    }).join('') || '<tr><td colspan="5" style="text-align: center; padding: 3rem; color: var(--text-secondary)">No hay equipos próximos a vencer en los próximos 180 días.</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
function updateAlertBadge() {
    ...
    const typeValves = state.valves.filter(v => v.assetType === state.currentAssetType);

    const expiredCount = typeValves.filter(v => v.state === 'VENCIDO').length;
    const upcomingCount = typeValves.filter(v => {
        if (v.state === 'VENCIDO') return false;
        const scheduledDate = parseSheetDate(v.scheduledDate);
        return scheduledDate && scheduledDate > now && scheduledDate <= alertWindowDate;
    }).length;
    ...
    if (alertBadge) {
        alertBadge.textContent = expiredCount;
        alertBadge.style.display = expiredCount > 0 ? 'inline-block' : 'none';
        alertBadge.style.background = 'var(--danger)';
    ...
    if (upcomingBadge) {
        ...
        upcomingBadge.style.display = upcomingCount > 0 ? 'inline-block' : 'none';
        upcomingBadge.style.background = 'var(--warning)';
        upcomingBadge.style.color = '#000';
    }
}
